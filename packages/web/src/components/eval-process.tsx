...
/**
 * Renders a challenge row with an expandable eval process below it.
 * The process expands as a full-width row spanning all columns.
 */
export function ChallengeRow({
  ...
  const steps = challenge.evalSteps;
    ...
    <>
      ...
      {hasSteps && (
        <tr>
          <td
            colSpan={colCount}
            style={{ padding: expanded ? '0 1rem 1rem' : '0 1rem 0.25rem', borderTop: 'none' }}
          >
            <button
            ...
            {expanded && (
              <div className="eval-process" style={{ marginTop: '0.5rem' }}>
                {steps!.map((step, i) => (
                  <div key={`${challenge.challengeId}-${step.name}-${i}`} className="eval-step">
...
/** Kept for backward compat, but ChallengeRow is preferred */
export function EvalProcess({ steps, challengeId }: { steps: EvalStep[]; challengeId: string }) {
  return null; // Replaced by ChallengeRow
}
